// Archive Preview 3 — "뉴스피드형"
// 왼쪽: 카드 박스 없음. 작성자 헤더(아바타+이름+기간) → 제목(큰 타이포) → 본문 미리보기. 구분선으로 분리.
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          {grouped.length === 0 ? (
            ...
            grouped.map((yg) => (
              <div key={yg.year}>
                <div className="px-4 py-2 text-[11px] font-black text-stone-300 tracking-widest uppercase bg-stone-50 border-b border-stone-100">
                  {yg.year}
                </div>
                {yg.months.map((mg) => (
                  <div key={mg.session}>
                    <div className="px-4 py-1.5 text-xs font-semibold text-stone-400 bg-stone-50/50 border-b border-stone-100 flex items-center justify-between">
                      <span>{mg.label}</span>
                      <span className="text-stone-300">
                        {mg.articles.length}
                      </span>
                    </div>
                    {mg.articles.map((article, idx) => {
                      const isSelected = article.id === selectedArticleId;
                      const nick = article.checkin_members.nickname;
                      const isLast = idx === mg.articles.length - 1;
                      return (
                        <button
                          key={article.id}
                          onClick={() =>
                            setSelectedArticleId(isSelected ? null : article.id)
                          }
                        ...
                        >
                          ...
                          {/* 제목 */}
                          <div
                            className={`text-[15px] font-bold leading-snug line-clamp-2 mb-1.5 ${
                              isSelected ? "text-white" : "text-stone-900"
                            }`}
                          >
                            {article.title}

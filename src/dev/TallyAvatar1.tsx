/**
 * 득표 현황 아바타 — Option 1: 스택 아바타 (Stacked Overlap)
 * - 우측에 겹쳐진 원형 아바타 (max 5개 → +N 오버플로우)
 * - 현재 row 레이아웃 그대로, 도트만 교체
 * - 나중에 avatar img src 추가 시 아바타 컴포넌트만 교체
 */
...
function Avatar({
  ...
  return (
    <div
      className={`${sizeClass} ${colorClass} rounded-full flex items-center justify-center text-white font-bold flex-shrink-0 ring-2 ring-white`}
    >
      {name[0]}
    </div>
  );
...
export default function TallyAvatar1() {
  return (
    <>
      <PhoneFrame>
        {/* 배경: 투표 페이지 블러 처리 */}
              ...
              <div className="p-3 max-h-72 overflow-y-auto">
                <div className="space-y-2">
                  {MOCK_TALLY.map((item) => {
                    ...
                    const overflow = voters.length - 5;
                    return (
                      <div
                          ...
                          <div className="flex">
                            {shown.map((v, i) => (
                              ...
                              >
                                <Avatar
                                  name={v.name}
                                  colorClass={AVATAR_COLORS[v.idx]}
                                />
                              </div>
                            ))}
                            {overflow > 0 && (
